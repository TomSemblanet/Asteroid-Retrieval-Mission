...
class Constraints:
    ...
    def build_constraints_boundaries(self):
        ...
        defects_low = np.zeros(
            self.problem.prm['n_states'] * 2 * (self.problem.prm['n_nodes']-1))
        defects_upp = np.zeros(
            self.problem.prm['n_states'] * 2 * (self.problem.prm['n_nodes']-1))

        # States path lower and upper boundaries 
        st_path_low = np.hstack([self.problem.low_bnd.st_path]
                             * (2*self.problem.prm['n_nodes']-1))
        st_path_upp = np.hstack([self.problem.upp_bnd.st_path]
                             * (2*self.problem.prm['n_nodes']-1))

        # Controls path lower and upper boundaries 
        ct_path_low = np.hstack([self.problem.low_bnd.ct_path]
                             * (4*self.problem.prm['n_nodes']-3))
        ct_path_upp = np.hstack([self.problem.upp_bnd.ct_path]
                             * (4*self.problem.prm['n_nodes']-3))
        ...
        low = np.concatenate((defects_low, st_path_low, ct_path_low, event_low))
        upp = np.concatenate((defects_upp, st_path_upp, ct_path_upp, event_upp))
    ...
    def compute_constraints(self, decision_variables_vector):
        ...
        t_i, t_f, f_prm, states, controls, states_add, controls_add, controls_col = utils.unpack_decision_variable_vector(
        ...
        # Computation of the defects constraints and conversion into a 1D-array
        defects_matrix = self.tr_method.compute_defects(
        	states, controls, states_add, controls_add, controls_col, f_prm, self.problem.dynamics,
        		self.problem.prm['sc_factor'])
       	defects = defects_matrix.flatten(order='C')

        # Computation of path constraints and conversion into a 1D-array
        if (self.problem.prm['n_st_path_con'] != 0 or self.problem.prm['n_ct_path_con'] != 0):
            st_path, ct_path = self.problem.path_constraints(
                states, controls, states_add, controls_add, controls_col, f_prm)

        if self.problem.prm['n_st_path_con'] != 0:
            st_path = st_path.flatten(order='F')
        else:
            st_path = np.empty(0)

        if self.problem.prm['n_ct_path_con'] != 0:
            ct_path = ct_path.flatten(order='F')
        else:
            ct_path = np.empty(0)
        ...
        con = np.concatenate((defects, st_path, ct_path, event))
