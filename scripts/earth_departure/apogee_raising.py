...
from scripts.earth_departure.utils import kepler_thrust, kep2cart, cart2kep, moon_reached, R2, apside_pass, plot_env_2D
...
def last_apogee_pass_time(r0, T, eps):
...
def propagate_to_last_apogee_pass(r0, tau, T, eps):
...
def last_arc_search(r_ap, v_inf, T, eps):
...
def propagate_last_branch(r_ap, T, eps_l):
	""" Search the last thrust arc length to encounter the Moon with the desired excess velocity using the secants
		method.
	...
	sol = solve_ivp(fun=kepler_thrust, y0=r_ap[:, -1], t_span=t_span, t_eval=t_eval, args=(T, eps_l), events=(moon_reached), rtol=1e-12, atol=1e-13)
...
def apogee_raising(T, eps, r_p, v_inf):
	...
	# ---------------------------------------------------------------------------------
	last_ap_pass_time = last_apogee_pass_time(r0=r0, T=T, eps=eps)
	print("Date of last apogee pass : {} sec".format(last_ap_pass_time))


	# 3 - Computation of the S/C states after the last apogee pass date
	# -----------------------------------------------------------------
	r_ap, t_ap = propagate_to_last_apogee_pass(r0=r0, tau=last_ap_pass_time, T=T, eps=eps)


	# 4 - Computation of the last arc semi-angle to reach the Moon with the desired excess velocity
	# ---------------------------------------------------------------------------------------------
	eps_l = last_arc_search(r_ap=r_ap, v_inf=v_inf, T=T, eps=eps)
	print("Last semi-angle : {}°".format(eps_l * 180 / np.pi))


	# 5 - Simulation of the last branch until Moon encounter
	# ------------------------------------------------------
	r_lb, t_lb = propagate_last_branch(r_ap=r_ap, T=T, eps_l=eps_l)
