...
class NEA2Earth:
	...
	def __init__(self, nea, n_seg, t0, tof, m0, Tmax, Isp, nea_mass, earth_grv=True):
		...
		self.earth_dist = 300000e3 # 400,000 km 
	...
	def propagate(self, x):
		...
		rf = r_e - (v_e / np.linalg.norm(v_e)) * self.earth_dist
		vf = v_e
	...
	def plot_traj(self, x):
		...
		ax.scatter((r_e[0] - (v_e[0] / np.linalg.norm(v_e)) * self.earth_dist) / pk.AU, (r_e[1] - (v_e[1] / np.linalg.norm(v_e)) * self.earth_dist) / pk.AU, \
			(r_e[2] - (v_e[2] / np.linalg.norm(v_e)) * self.earth_dist) / pk.AU, marker='+', s=10, color='green')
