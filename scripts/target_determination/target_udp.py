...
class Earth2NEA:
	...
	def __init__(self, nea, n_seg, tf, tof, m0, Tmax, Isp, earth_grv=True):
		...
		# Boundaries 
		# [<arrival date>, <time of flight>, <final mass>, <throttle[0]>, ..., <throttle[n_seg-1]>]
		self.lb = [tf[0].mjd2000] + [tof[0]] + [0] + [-1, -1, -1] * n_seg
		self.ub = [tf[1].mjd2000] + [tof[1]] + [m0] + [1, 1, 1] * n_seg
	...
	def propagate(self, x):
		...
		# Computation of the initial ephemerides (Departure from the Earth)
		ti = pk.epoch(t0)
