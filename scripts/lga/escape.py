...
from scripts.lga.utils import initial_states, plot_env, moon_orbit_intercept, earth_collision, moon_rot_matrix
...
def second_LGA(cr3bp, v_inf, alpha, theta_s0):
	...
	ax.plot([r_M_f[0], r_M_f[0]+i[0]/500], [r_M_f[1], r_M_f[1]+i[1]/500], ':')
	ax.plot([r_M_f[0], r_M_f[0]+j[0]/500], [r_M_f[1], r_M_f[1]+j[1]/500], ':')
	...
	# ------ Real pump angle ------
	pp = np.arccos(np.dot(r_M_f[3:], V_m_inf) / (np.linalg.norm(V_m_inf)*np.linalg.norm(r_M_f[3:])))
	print("Real pump angle : {}°".format(pp*180/np.pi))

	# S/C Velocity at infinity in the rotated frame before LGA [-]
	# /!\ Si P est la matrice de passe de B à B' alors un vecteur X
	...
	# ax.plot([0, 0], [0, V_M/10])
	# ax.plot([0, V_m_inf_rot[0]/10], [0, V_m_inf_rot[1]/10])
	# ax.plot([V_m_inf_rot[0]/10], [0], 'o')
	# ax.plot([0], [V_m_inf_rot[1]/10], 'o')

	# Pump-angle before the LGA
	if V_m_inf_rot[1] >= 0:
		p_m = np.arctan(V_m_inf_rot[0]/V_m_inf_rot[1])
	else:
		p_m = np.pi - abs(np.arctan(V_m_inf_rot[0]/V_m_inf_rot[1]))

	# Maximum rotation angle (delta_max)
	delta_max = 2 * np.arcsin( mu_M/rp / ((V_inf_mag*cr3bp.V)**2 + mu_M/rp) )

	# Pump angle (+) after LGA interval
	p_p_range = np.linspace(p_m - delta_max, p_m + delta_max, 10)
	...
	for p_p in p_p_range:
		...
		k_p_range = np.linspace(0, k_p_max, 1)
		...
		for k_p in k_p_range:
			...
			# V_p_X = V_p_SC_rot[0] * np.cos(-theta) + V_p_SC_rot[1] * np.sin(-theta)
			# V_p_Y = -V_p_SC_rot[0] * np.sin(-theta) + V_p_SC_rot[1] * np.cos(-theta)
			# V_p_Z = V_p_SC_rot[2]
			# V_p_SC = np.array([V_p_X, V_p_Y, V_p_Z])
		...
		rtol=1e-13, atol=1e-12)
			...
			if t_[-1] == t_span_[-1]:
				ax.plot(r_[0], r_[1], color='blue')


	ax.plot(r[0], r[1], color='blue')
	ax.plot([r[0, -1]], [r[1, -1]], 'o', color='m', markersize=3)

	plot_env(ax, cr3bp, theta_s0, "Feasible double LGA")
