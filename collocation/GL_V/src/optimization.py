...
from collocation.GL_V.src.transcription import Transcription
from collocation.GL_V.src.solver import IPOPT
...
class Optimization:
    ...
    def run(self):
        ...
        opt_st, opt_ct, opt_pr, opt_tm, total_ct = self.solver.launch()

        # Once optimization is finished, we post-process the results
        self.post_process(opt_st, opt_ct, opt_pr, opt_tm, total_ct)

    def post_process(self, opt_st, opt_ct, opt_pr, opt_tm, total_ct):
        ...
        self.results['total_ct'] = total_ct
    ...
    def explicit_integration(self):
        ...
        self.transcription.tr_method.build_interpolation_func(
                self.results['opt_tm'], self.results['opt_ct'])

        self.results['int_st'] = utils.explicit_integration(
            self.results['opt_tm'], self.results['opt_st'][:, 0], self.results['opt_ct'], \
            self.results['opt_pr'], self.problem.dynamics, self.transcription.tr_method.interpolate_ctrl)
