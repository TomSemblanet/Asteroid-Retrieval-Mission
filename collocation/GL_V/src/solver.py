...
from collocation.GL_V.src import constraints as cs, cost as ct, collocation as col, utils
...
class IPOPT:
    ...
    def launch(self):
        ...
        t_i, t_f, f_prm, opt_states, opt_controls, opt_states_add, opt_controls_add, opt_controls_col = \
        	utils.unpack_decision_variable_vector(_x, problem.prm)

        # Reconstruction of the time grid
        time_grid = utils.retrieve_time_grid(problem.prm['h'], t_i, t_f)

        # Construction of the total controls array (concatenation of the controls at
        # generic, additional and collocation nodes) used for explicit integration
        total_controls = utils.build_final_controls_vec(opt_controls, opt_controls_add, opt_controls_col)

        return opt_states, opt_controls, f_prm, time_grid, total_controls
