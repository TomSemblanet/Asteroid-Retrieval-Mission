...
from collocation.GL_V.src import constraints as cs, cost as ct, \
    collocation as col, scaling as sc, utils
...
class Transcription:
    ...
    def __init__(self, problem, options):
        ...
        # Adaptation of the time grid
        self.grid_adaptation()

        # Decision variable vector construction
        self.decision_variables_vector = self.build_decision_variable_vector()
    ...
    def grid_adaptation(self):
        """ Scales the time so it belongs to the interval [-1, 1] and computes 
        ...
        self.tr_method = col.GL5(self.problem.prm.copy())
    ...
    def build_decision_variable_vector(self):
        ...
        dvv = utils.make_decision_variable_vector(self.problem.initial_guess.states, self.problem.initial_guess.controls, \
            self.problem.initial_guess.states_add, self.problem.initial_guess.controls_add, \
            self.problem.initial_guess.controls_col, self.problem.initial_guess.time[0], \
            self.problem.initial_guess.time[-1], self.problem.initial_guess.f_prm, \
            self.problem.prm)
    ...
    def build_decision_variable_vector_boundaries(self):
        ...
        # Additional states boundaries initialization
        states_add_low = np.hstack(
            [self.problem.low_bnd.states] * (self.problem.prm['n_nodes']-1))
        states_add_upp = np.hstack(
            [self.problem.upp_bnd.states] * (self.problem.prm['n_nodes']-1))

        # Additional controls boundaries initialization
        controls_add_low = np.hstack(
            [self.problem.low_bnd.controls] * (self.problem.prm['n_nodes']-1))
        controls_add_upp = np.hstack(
            [self.problem.upp_bnd.controls] * (self.problem.prm['n_nodes']-1))

        # Collocation controls boundaries initizalization
        controls_col_low = np.hstack(
            [self.problem.low_bnd.controls] * (2*(self.problem.prm['n_nodes']-1)))
        controls_col_upp = np.hstack(
            [self.problem.upp_bnd.controls] * (2*(self.problem.prm['n_nodes']-1)))

        # Concatenation of the states, controls, controls-mid, free-parameters, initial and final time boundaries
        low = np.concatenate((states_low, controls_low, states_add_low, controls_add_low, controls_col_low, self.problem.low_bnd.f_par,
                              [self.problem.low_bnd.ti], [self.problem.low_bnd.tf]))
        upp = np.concatenate((states_upp, controls_upp, states_add_upp, controls_add_upp, controls_col_upp, self.problem.upp_bnd.f_par,
